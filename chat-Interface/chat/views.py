...
def send_data_to_fastapi(request):
    ...
    fastapi_url = "http://52.67.197.89:8000/generate_pdf/"  # Cambia si FastAPI está en otra dirección
...
def api_chat(request):
    if request.method == 'POST':
        try:
            ...
            url = 'http://52.67.197.89:8000/chat/'  # URL de la API de FastAPI
...
def api_view(request):
    if request.method == "POST":
        ...
        try:
            response = requests.post('http://52.67.197.89:8800/chat/', json={'user_input': user_input})
            if response.status_code == 200:
